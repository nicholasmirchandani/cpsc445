...
So Speedup would be (m * tX) / (log(p) * ((20 + m/p) * tB + 6 * tL) + m/p * tX)
...
Assuming best topology for each step, despite not using BCast in my code, runtime for comms is log(p) * (tB * 4 + tL) + log(p) * (tB * m + tL) + log(p) * (tB * m + tL) = log(p) * ((6 * m) * tB + 3 * tL)

Of course don't forget to add the m/p * tX of parallel computation

So Speedup would be (m * tX) / (log(p) * ((6+m) * tB + 3 * tL) + m/p * tX)
